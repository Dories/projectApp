...
/**   ============================================ 执行SQL update语句   ============================================= */
/**
 *  @brief  执行SQL update语句
 *
 *  @param  key    {"databaseName":"","sql":""}
 *  @param  success_callback    成功回调
 *  @param  failed_callback"    失败回调
 *
 *  @return 成功返回1 失败返回0
 */
function executeUpdateSql (key,success_callback,failed_callback) {
    // iOS
    if(brows().iphone) {
        cordova.exec(success_callback,failed_callback,"TableDataPlugin","executeUpdateSql",[key]);
    }
    // Android
    else if(brows().android) {
        return  cordova.exec(success_callback,failed_callback,"TableDataPlugin","executeUpdateSql",key);
    }
}
...
/**   ============================================ 批量插入表单数据   ============================================= */
/**
 *  @brief {"databaseName":"","tableName":"","data":""}
 *
 *  @param  success_callback    成功回调
 *  @param  failed_callback"    失败回调
 *
 *  @return 批量插入表单数据成功返回1  批量插入表单数据返回0
 */
function syncInsertTableData(key,success_callback,failed_callback) {
    // iOS
    if(brows().iphone) {
        cordova.exec(success_callback,failed_callback,"TableDataPlugin","syncInsertTableData",[key]);
    }
    // Android
    else if(brows().android) {
        return  cordova.exec(success_callback,failed_callback,"TableDataPlugin","syncInsertTableData",key);
    }
}
 ...
 */
function uploadImage(uploadURL, filePath, agentCode, pwd, time, success_callback, failed_callback) {
    // iOS
    if(brows().iphone) {
        cordova.exec(success_callback, failed_callback, "PhotoPlugin", "uploadImage", [uploadURL, filePath, agentCode, pwd, time]);
    }
    
    // Android
    else if(brows().android) {
        return  cordova.exec(success_callback, failed_callback, "UpLoadImagePlugin", "uploadimage", [uploadURL, filePath, agentCode, pwd, time]);
    }
};
